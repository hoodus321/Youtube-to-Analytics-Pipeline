...
def transcribe_audio_files_in_directory(directory_path):
    ...
    for mp4s in all_mp4:
        ...
        srt_path = mp4s_path.with_suffix('.srt')

        # Check if the .srt file already exists
        if srt_path.exists():
            print(f"Skipping {mp4s}, .srt file already exists. ")
            continue

        # Convert the current .mp4 file to a string as a parameter for the .transcribe() function
        ...
        print(f"Transcribed: {mp4s}")




# Specifying the root directory that contains all the channel and playlist folders
root_directory_path = r"/Users/hadibhidya/Desktop/new_samples"
